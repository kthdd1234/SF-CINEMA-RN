...
const stackScreens = [
  ...
  { name: 'SignUp', component: SignUp, options: Object.assign({ title: '회원가입' }, headerStyle)}
]
        ...
        <Stack.Navigator initialRouteName='TabScreens'>
          {
            stackScreens.map((screen, screenIdx) => (
              <Stack.Screen
                ...
                options={({ route }) => (screen.name === 'MovieCollection' ? Object.assign({ title: route.params.headerTitle }, screen.options) : screen.options)}
              />
            ))
          }
        </Stack.Navigator>
